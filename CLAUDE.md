...
Guidance for Claude Code working in this repo. **This file is rules + map only.** Detailed,
...
| Was the case actually *closed*? (verdict + mandatory contradictions + false stamps) | `src/engine/caseSuccessEngine.ts` |
...
`CLAUDE.md` only if the doc map or an invariant shifted. Edit surgically (touch affected lines, don't
