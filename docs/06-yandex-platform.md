...
`GAME_CONFIG.saveVersion` — текущая версия схемы персиста (сейчас **12**). `migrate()` в
...
- v11 → v12: добавлен `firstSeenServerDay` — маркер первой сессии, от которого считается
  оффер `next_day` ([04-economy-progression.md](04-economy-progression.md)). Бэкфиллится в
  `null`, а стор проставляет его на первой загрузке после апгрейда: у вернувшегося игрока
  next-day оффер откроется через день после этой загрузки, а не задним числом. Это
  осознанно — иначе пришлось бы выдумывать день, которого профиль никогда не помнил.
...
**Очередь облачной записи.** `pushToCloud()` работает как drain-loop: пока `pendingSnapshot`
не пуст, снапшоты пишутся по одному, а `flushSync` присоединяется к уже идущей записи вместо
раннего выхода. Это обязательно для IAP: одна выдача бандла порождает несколько `flushSync`
в одном тике (`grantArchivePurchases` → `grantStampTextPurchases` → `grantBundlePurchase`), и
