...
Дело закрывается **одним** листом — человек и награда в одном окне, без «продолжить» между ними
(раньше реакция жила в отдельном `CaseResolutionSheet`, и каждое дело заканчивалось дважды).
Порядок сверху вниз:

1. **Штамп исхода** — один на весь лист, четыре варианта (`fraudExposed` / `caseClosed` /
   `fraudMissed` / `investigatorError`), плюс название дела и подзаголовок исхода.
2. **Что почувствовал человек?** — портрет заявителя (`personImage`), его единственная реплика
   (`resolution.finalLine`) и, ровно в 15 делах из 56, одна строка Веры (`resolution.veraLine`).
3. **Чек** — гонорар/убыток крупно, чипы XP + мастерство + бонус, «удвоить награду» сразу под
   суммой. Деньги стоят рядом с репликой, а не в конце листа: на телефоне за ними иначе
   пришлось бы скроллить, и рекламный CTA уезжал бы вместе с ними.
4. **Точность** — шкала и разбор по строкам (победа) либо ящик «Что вы упустили» (проигрыш):
   **закрыт по умолчанию**, заголовок красный, открывается и закрывается на месте.
5. **Почему решение верное?** — такой же сворачиваемый ящик «Почему такое решение?»
   (тоже закрыт по умолчанию):
   ...
   к конкретным уликам через `evidenceIds`. Стоит сразу за «что вы упустили» и показывается
   и при **проигрыше** — это единственное объяснение дела, которое игрок получает после ошибки.
   У дел без авторского разбора (архивные паки — `resolution` там нет вовсе) под той же кнопкой
   раскрываются строки `caseData.explanation`.
6. **Запись в Архиве №17** — `resolution.arcReveal`, визуально отделённая пунктирная карточка.
   Появляется только в сюжетных делах (28 из 56), затем повышение и ачивки. Ряд кнопок закреплён
   внизу листа и не уезжает со скроллом.

Слои 2 и 6 показываются **только при верном** вердикте: часть реплик содержит косвенное
признание и выдала бы разгадку до пересмотра дела. Это гарантируется схемой
(`resolution.verdict` обязан совпадать с `correctDecision`) и `caseResolution.test.ts`.

Отдельного блока «Истина по делу» и плитки «Бюджету спасено» на листе больше нет: дело
объясняет разбор по кнопке, а сумма спасённого бюджета дублировала гонорар. Одна лента
раскрытия на весь лист (штамп → реплика → сумма → шкала → трофеи), ~1.7 с до полной остановки.
