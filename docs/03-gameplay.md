...
## Финал дела: одна модалка (`ResultSheet`)

Дело закрывается **одним** листом (папка на столе, а не два экрана): режим выбирает
`result.success` — см. `src/engine/caseSuccessEngine.ts`. Верстка собрана по утверждённым
референсам `big-update/detective_final_windows_package/`.
...
+ ошибочных штампов не больше GAME_CONFIG.caseSuccess.maxFalseStamps
= дело успешно завершено
```

«Обязательные противоречия» — карточки с **обоими** флагами `isContradiction` и
`requiredForVerdict` (необязательные противоречия стоят точности и денег, но никогда — прогресса).
Только закрытое дело попадает в `stats.completedCaseIds`, а его читает `caseUnlockEngine` — значит,
проваленное дело больше не открывает следующее. Rewarded-видео **никогда** не входит в предикат:
реклама покупает подсказку, не прогресс. Настройки предиката — `GAME_CONFIG.caseSuccess`.

Отсюда три состояния листа:

| Состояние | Когда | Основное действие |
| --- | --- | --- |
| `solved` | предикат выполнен | `Следующее дело →` |
| `incomplete` | вердикт верный, обязательных улик не хватает | `↻ Повторить без подсказки` |
| `failure` | вердикт неверный | `↻ Повторить без подсказки` |

### Состояние успеха

Сверху вниз: таб папки «Дело №N · Закрыто» → статус (иконка, kicker, заголовок исхода, название
дела, компактный штамп «Обосновано») → **финансовый результат** крупно (+ чип мастерства) →
точность и шкала → карточка ключевой улики → чипы XP и серии (только реально существующие
данные) → золотая rewarded-кнопка «Удвоить награду» с точной суммой бонуса → сворачиваемый
«Короткий разбор решения» → запись в Архиве, повышение, ачивки. В подвале — вторичное
`К столу` и основной CTA `Следующее дело →`: rewarded-кнопка заметна, но никогда не громче
основного перехода.

Разбор (`Drawer`, закрыт по умолчанию) держит то, что раньше занимало пол-листа: реплику
заявителя (`resolution.finalLine`), `resolution.reasoningChain` (или строки `caseData.explanation`
у архивных дел, где `resolution` нет) и строку Веры. Реплика показывается **только** при закрытом
деле: часть реплик содержит косвенное признание и выдала бы разгадку до пересмотра. Портрета
заявителя на листе больше нет.

### Состояние ошибки и rewarded-подсказка

Бесплатно даётся только общий сигнал «вы упустили важное противоречие» — конкретная улика скрыта
за золотой карточкой «Пропущенная улика» с rewarded-кнопкой «Открыть улику». В состоянии
`incomplete` та же карточка стоит под строкой «Обязательные улики: N из M».

Открывается ровно одна карточка — первое пропущенное обязательное противоречие в авторском
порядке (`clueEvidenceFor`), поэтому при повторном открытии листа это всегда та же улика. Успешный
просмотр пишет `stats.caseClueReveals[caseId] = evidenceId` (`revealMissedClue`, идемпотентно),
и `startCase` кладёт этот id в `session.revealedEvidenceIds` — обещание «Вернуться к делу с
подсказкой» переживает и повтор, и перезагрузку. Cancel / error / no-fill подсказку не дают:
кнопка восстанавливается, сообщение объявляется через `aria-live`.

Бесплатная альтернатива никогда не за рекламой — `↻ Повторить без подсказки` закреплена в подвале,
чтобы её нельзя было укатать скроллом на 360×640. Отдельная неинтерактивная плашка «Следующее дело
пока закрыто» объясняет условие. Кнопка «Следующее дело» в этих состояниях **не рисуется вовсе**.
...
- `Unlock next case with an ad` навсегда открывает одно следующее дело пака и пишет case id в
  `stats.archiveUnlockedCaseIds`; тот же rewarded-путь есть в диалоге покупки, но открывает
  **нажатое** дело («Открыть это дело за рекламу») — `unlockArchiveCaseViaAd` принимает любой
  case id пака, порядок не навязывается;
...
- на странице архива покупка всегда первична (одна золотая кнопка в карточке покупки), а
  бесплатный пробник — тихая ссылка под ней, которая пропадает после его прохождения; глагол
  кнопки при этом меняется на «Продолжить расследование»;
- **закрытая плитка дела — точка входа в покупку**: клик по ней открывает `ArchivePurchaseModal`
  (весь архив за одну цену либо rewarded-открытие именно этого дела). Сам диалог денег не берёт —
  подтверждение платежа остаётся в окне платёжного сервиса Yandex.
