...
`TopBar` — единственная сквозная хрома: бренд-марка «НЛ», «Расследование», кнопка «Бюро особых дел»
с печатью AR и бейджем NEW, ресурсные чипы (★ XP, ₽ баланс) и шестерёнка (restore purchases).
Ниже 720 px схлопывается: пропадают тэглайн, «Расследование», чип XP и шестерёнка, а кнопка Бюро
остаётся печатью с бейджем (accessible name сохраняется через `aria-label`).
...
(разбор награды), `LeftSidebar` / `RightSidebar`, `TopBar`, `SpecialArchivesEntry`,
