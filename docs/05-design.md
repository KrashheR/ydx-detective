...
`SpecialArchivesEntry` — карточка-вход: в десктопном `RightSidebar` через проп `archivesSlot`,
в `MobileDeskMenu` — в варианте `compact`. Витрина-каталог показывает слева список архивов-паков,
...
`CaseResolutionSheet` (реакция человека на вердикт), `ResultSheet`
(разбор награды), `LeftSidebar` / `RightSidebar`, `SpecialArchivesEntry`,
`ThematicPacksModal`, `AchievementsModal`, `StampShopModal`, `RatingModal`, `LanguageSelector`,
`Tooltip`, `icons.ts`.

`StampShopModal` — «Мастерская штампов», косметическая витрина подписей штампа противоречия
(тот же тёмный `folder-edge` заголовок и бумажный лист, что у `AchievementsModal`). Товар —
сам оттиск: каждая строка показывает превью печати (некупленные — приглушённые), кнопку с
живой ценой из каталога Yandex и «Поставить на печать» для купленных; внизу — restore.
Входы: кнопка под достижениями в `RightSidebar` (desktop) и строка над «Делом дня» в
`MobileDeskMenu`. Каталог подписей — `src/data/stampTexts.ts`; длинные подписи ужимают
кегль оттиска в `EvidenceCard` и `StampModal`, чтобы угловая лента и печать не разъезжались.

`CaseResolutionSheet` открывается **перед** `ResultSheet` после верного вердикта и держит тот же
кремовый лист. Сверху вниз: штамп вердикта (форма + пиктограмма + текст — никогда не один цвет),
портрет-досье 96 px с лёгким наклоном по `emotionalMode` (у режима `grave` наклон нулевой и
портрет приглушён), имя, реплика на карточке стенограммы с цветной левой линейкой — не в
мультяшном облаке. Реплика доступна целиком сразу: посимвольной печати нет. Ниже — опциональная
строка Веры, кнопка «Почему такое решение?» (разбор раскрывается на месте, не новой модалкой) и
пунктирная карточка «Новая запись в Архиве №17», визуально отделённая от страхового решения.
Сумм, процентов и XP на этом экране нет — они начинаются со следующего листа.
