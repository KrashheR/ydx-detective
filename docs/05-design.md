...
справа каталог подписей. Тап по некупленной подписи — примерка, повторный тап — покупка.
...
(`bundle.archives`) и «Отключение обязательной рекламы» (`noads.forever`, единственная точка
