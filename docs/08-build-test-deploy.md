...
```bash
npm install
npm run dev        # Vite dev-сервер. SDK авто-фолбэк на LocalStorage вне CrazyGames.
npm run build      # прод-бандл → dist/ (base './', готов к ZIP для CrazyGames)
npm run typecheck  # tsc --noEmit — статический гейт; гонять перед «готово»
npm test           # vitest run — полный прогон
npm run test:watch # vitest в watch-режиме
npm run test:cov   # vitest run --coverage
npm run metrika:goals          # dry-run синхронизации целей Метрики
npm run metrika:goals:publish  # создать отсутствующие цели Метрики
```
...
Для проверки любого дела без прохождения кампании используйте `npm run dev:all-levels`.
Команда включает dev-only режим: все 50 стандартных дел доступны сразу, включая дела,
которые обычно требуют уровень или завершение предыдущего. Обычный `npm run dev` и
production-сборки этот режим не включают.

## Деплой на CrazyGames

1. `npm run build`.
2. Загрузить **содержимое** `dist/` как ZIP в CrazyGames Developer Portal Preview.
3. Относительный base (`./`) гарантирует работу с хостинг-пути платформы.
4. Для Basic Launch убедиться, что `VITE_CRAZYGAMES_ADS_ENABLED` не задан: никакие
   rewarded/interstitial UI, IAP и искусственный лидерборд не должны появиться.
5. Добавить metadata и три обложки из `submission-assets/crazygames/`, затем проверить
   SDK lifecycle и mobile/desktop в Preview Portal.
...
## Campaign migration verification

After changing or re-importing campaign content, run:

```bash
node scripts/import-campaign-50.mjs
npm run typecheck
npm test
npm run build
```

`campaignProgression.test.ts` checks the exact 50-position roster, budgets and onboarding types;
`caseSchema.test.ts` validates content invariants; `InteractiveEvidence.test.tsx` exercises all five
renderers (including RTL and restored progress); `platformAdapter.test.ts` covers CrazyGames routing.
The production build must retain one lazy chunk per case rather than embedding the campaign in the
