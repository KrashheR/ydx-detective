...
| `type` | `standard \| daily` | daily → ×1.25 награда, ×2 XP, файл в `cases/daily/` |
| `difficulty` | `easy \| medium \| hard` | вес XP: 10 / 20 / 35 |
| `claimAmount` | number | сумма заявки в USD / база награды |
...
1. Добавь запись в `ACHIEVEMENTS` (`src/data/achievements.ts`): `id`, `icon` (сдержанный
   ...
   языков на компиляции), `xpBonus`, `currencyBonus`.
...
## Campaign 1–50 authoring contract

Every campaign case must include immutable `campaignOrder`, integer `requiredLevel` (≤16), numeric
`investigationBudget`, `claimStatements`, content/schema/revision versions and linked evidence.
Every evidence uses `statementLink.relation` (`supports`, `contradicts`, `contextualizes` or
`reveals_season_clue`) plus an `evidenceTier`; optional graph fields are
