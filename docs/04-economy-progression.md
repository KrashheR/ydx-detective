...
**Неверный вердикт → награда `0` целиком.** `evaluateReward` делает ранний выход, когда
`decision !== correctDecision`: все компоненты (вердикт, доказательства, эффективность, бонус)
обнуляются, штраф **не** начисляется (упущенная выплата — это и есть вся «цена» ошибки).
Награда — это привилегия верного вердикта. Таблица ниже описывает **только верный вердикт**:

| Компонент | Формула | Условие |
| --------- | ------- | ------- |
| Вердикт | `verdictShare × base` | начисляется (вердикт уже верный) |
...
| Эффективность | `efficiencyShare × base × (unusedOpens / budget)` | только бюджетные дела |
| Бонус | `(rank% + streak%)` от **позитивной** базы (вердикт+доказательства+эффективность) | никогда не применяется к штрафу |
| Штраф | `falseStampPenalty (50)` за каждую **ложно** заштампованную карточку | только при верном вердикте |
...
Прочее: `dailyMultiplier 5`. Итог (`total`) может быть **отрицательным** только при верном
вердикте, заваленном ложными штампами (штраф превышает позитив); неверный вердикт даёт ровно `0`.
