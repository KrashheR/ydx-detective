...
`BaseReward = difficultyBase × (daily ? 2 : 1)`, где easy = `1 200`, medium = `2 500`,
...
Прочее: `dailyMultiplier 2`. Итог (`total`) — это `Math.max(0, positive + bonusComponent -
...
`type === 'daily'` даёт ×2 к награде и ×2 к XP. Доступность считает
